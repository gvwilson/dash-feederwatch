...
def create_components(birds, species_labels, regions_labels):
    ...
    return {
        ...
        STORAGE: dcc.Store(storage_type='memory', id=STORAGE),
    }
...
def do_layout(name, components):
    ...
    app.layout = dmc.MantineProvider(children=[
        # storage (not shown, but must be in layout for callbacks to work)
        html.Div(components[STORAGE]),
        # title
...
def create_callbacks(birds):
    ...
    # Re-create graph and table when storage changes
    @callback(
        Output(DISPLAY_GRAPH, 'figure'),
        Output(DISPLAY_TABLE, 'data'),
        Input(STORAGE, 'data'),
    )
    def update_graph(storage):
        df = pd.DataFrame(json.loads(storage))
        fig = px.scatter(df, x='region', y='species_id', size='num')
    ...
    # Update storage when pulldowns change
    @callback(
        Output(STORAGE, 'data'),
        Input(CHOOSE_REGION, 'value'),
        Input(CHOOSE_SPECIES, 'value')
    )
    def update_storage(region, species):
